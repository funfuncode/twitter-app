...
class TwitterController extends Controller
{
   ...
   private $format = 'json';
    ...
    public function twitterUserTimeLine () {
      $data = json_decode(Twitter::getUserTimeLine( ['count' => $this->count, 'format' => $this->format] ));

      return view('twitter')->with('data', $data);
   }

   public function homeTimeLine() {
      $data = json_decode( Twitter::getHomeTimeLine( ['count' => $this->count, 'format' => $this->format ] ) );

      //return dd($data);
      return view('hometimeline', compact('data'));
   }
